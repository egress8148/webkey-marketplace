...
export default function HomePageShell() {
  ...
  const quoteDao2Fallback = useMemo(() => {
    ...
    // runtime_config 에서 환율 가져오기 (ratio: 1 USDT = ? DAO2)
    const ratio = Number(
      (runtimeConfig?.prime as { dao2_per_usdt?: number | string } | undefined)?.dao2_per_usdt ||
      (runtimeConfig?.marketplace as { dao2_per_usdt?: number | string } | undefined)?.dao2_per_usdt ||
      1
    ) || 1
    // priceUsdt6 is 6-decimal integer; convert to DAO2 18-decimal bigint
    const usdt = Number(quotePriceUsdt6) / 1_000_000  // actual USDT
    const dao2 = usdt * ratio
    return BigInt(Math.round(dao2 * 1e18))  // return as bigint in 18-decimals
  }, [buyingItem, quotePriceUsdt6, runtimeConfig])
  const quoteDao2Amount = quoteDao2Data ?? quoteDao2Fallback
  ...
  // ✨ Patch v15.1: 구매 완료 시 토스트 ("내 주문" 탭 바로가기 추가)
  useEffect(() => {
    if (buyStep === 'done') {
      toast.custom({
        kind: 'success',
  ...
  const handleSellerDeleteProduct = async (productId: string, title: string) => {
    ...
    try {
      // ✨ Patch v17.0: 토스트 추가 + HTTP 상태 체크
      const res = await fetch('/api/seller/product-manage', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'x-wallet-address': address,
        },
        body: JSON.stringify({ wallet: address, productId }),
      })
  ...
  const handleUpdateRequestStatus = async (
    ...
    try {
      ...
      if (nextStatus === 'Approved') toast.success('상품 승인 완료', '구매 목록에 노출됩니다.')
      else if (nextStatus === 'Rejected') toast.custom({ kind: 'error', title: '상품 거절', description: rejectionReason || '사유 없음' })
  ...
  const handleDeleteProductImage = async () => {
    ...
    if (!confirm('대표 이미지를 삭제할까?')) return
            ...
            <div className="rounded-3xl bg-white/85 backdrop-blur-2xl border border-purple-300/70 shadow-2xl shadow-purple-200/60 p-8">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2 rounded-2xl bg-slate-100/80 p-1">
                  ...
                  >📋 내 주문 {myBuyOrders.length > 0 && (<span className="ml-1 rounded-full bg-violet-500 px-2 py-0.5 text-[10px] text-white">{myBuyOrders.length}</span>)}</button>
                </div>
              </div>
              <p className="mt-3 text-sm text-slate-700">
                {buyerTab === 'shop' ? '승인된 상품 목록과 운영 설정을 서버 API 기준으로 불러온다.' : '내가 구매한 주문의 진행 상태를 확인합니다.'}
              </p>
              ...
              {buyerTab === 'orders' && (
                <div className="mt-6 space-y-4">
                  ...
                  {isConnected && myBuyOrdersLoading && (
                    <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">주문 로드 중...</div>
                  )}
                  {isConnected && !myBuyOrdersLoading && myBuyOrders.length === 0 && (
                    <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">
                      아직 구매한 주문이 없습니다. 상품 구매 탭에서 상품을 구매해보세요.
                    </div>
                  )}
                  {isConnected && !myBuyOrdersLoading && myBuyOrders.length > 0 && (
                    <div className="grid gap-4">
                              ...
                              <div className="flex flex-col items-end gap-2">
                                ...
                                {canCancel && (
                                  <button onClick={() => handleCancelMyOrder(od.order_id)}
                                    className="rounded-lg bg-rose-100 px-3 py-1 text-xs font-bold text-rose-700 hover:bg-rose-200">주문 취소</button>
                                )}
                              </div>
                            </div>
                          </div>
          ...
          <div className="relative">
            ...
            <div className={`${sellTab === 'mine' ? 'block' : 'hidden'} space-y-6`}>
              ...
              <div className="flex gap-2 rounded-2xl bg-slate-100/80 p-1">
                ...
                >📦 들어온 주문 {mySellOrders.length > 0 && (<span className="ml-1 rounded-full bg-violet-500 px-2 py-0.5 text-[10px] text-white">{mySellOrders.length}</span>)}</button>
              </div>
              ...
              {mineSubTab === 'incoming_orders' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-black text-slate-800">📦 들어온 주문 관리</h3>
                    <button onClick={() => loadMySellOrders()}
                      className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200">🔄 새로고침</button>
                  </div>
                  {!isConnected && (
                    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">지갑을 연결하세요.</div>
                  )}
                  {isConnected && mySellOrdersLoading && (
                    <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">주문 로드 중...</div>
                  )}
                  {isConnected && !mySellOrdersLoading && mySellOrders.length === 0 && (
                    <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">아직 들어온 주문이 없습니다.</div>
                  )}
                  {isConnected && !mySellOrdersLoading && mySellOrders.length > 0 && (
                    <div className="grid gap-4">
                      {mySellOrders.map((od) => {
                        ...
                        const statusLabel: Record<string,string> = {
                          pending: '대기', confirmed: '결제완료', reserved: '예약확정',
                          shipped: '배송중', delivered: '배송완료', completed: '거래완료',
                          cancelled: '취소', sent: '전송완료',
                        }
                        const pt = od.product_type || 'physical'
                        const typeEmoji: Record<string,string> = { physical: '📦', service: '📅', food: '🍽', digital: '💾' }
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                              ...
                              {pt === 'service' && od.status === 'confirmed' && (
                                <button onClick={() => handleUpdateOrderStatus(od.order_id, 'reserved')}
                                  className="rounded-lg bg-violet-500 px-3 py-1 text-xs font-bold text-white">예약 확정 (이메일 전송)</button>
                              )}
                              {pt === 'food' && od.status === 'confirmed' && (
                                <button onClick={() => handleUpdateOrderStatus(od.order_id, 'reserved')}
                                  className="rounded-lg bg-amber-500 px-3 py-1 text-xs font-bold text-white">수령 확정 (이메일 전송)</button>
                              )}
              ...
              {/* ✨ Patch v15.1: 기존 '내 등록 요청 현황' 본문은 mineSubTab==='requests'일 때만 노출 */}
                ...
                <div className="mt-6 space-y-4">
                  ...
                  {productType === 'physical' && (
                    <div className="grid gap-4">
                      ...
                      {/* ✨ Patch v16.1: 사이즈 가이드 */}
                      <div>
                        <label className="mb-2 block text-sm text-slate-800">📏 사이즈 가이드 (구매자에게 표시됩니다)</label>
                        <textarea
                  ...
                  {productType === 'service' && (
                    <div className="grid gap-4">
                      ...
                      <div className="rounded-2xl border-2 border-violet-200 bg-violet-50/40 p-4 space-y-3">
                        <div className="text-sm font-black text-violet-700">📝 운영 정보 (구매자 표시)</div>
                        <div>
                          <label className="mb-1 block text-xs font-semibold text-slate-700">📍 서비스 주소 <span className="text-red-500">*</span></label>
                          <input
                  ...
                  {productType === 'food' && (
                    <div className="grid gap-4">
                      <div>
                        <label className="mb-2 block text-sm text-slate-800">📍 매장 주소 <span className="text-red-500">*</span></label>
                        <input
                      ...
                      <div>
                        <label className="mb-2 block text-sm text-slate-800">⏰ 영업시간 <span className="text-red-500">*</span></label>
                        <input
            ...
            {isAdmin && (
              <div className={`${sellTab === 'admin' ? 'block' : 'hidden'} space-y-6`}>
                ...
                <div className="flex flex-wrap gap-2 rounded-2xl bg-slate-100/80 p-1">
                  {([
                    { key: 'review',     label: '📝 상품 검토' },
                    { key: 'rejected',   label: '❌ 거절된 상품' },
                    { key: 'categories', label: '📂 카테고리 관리' },
                    { key: 'mail',       label: '📨 이메일 큐' },
                ...
                {adminSubTab === 'rejected' && (
                  <div className="rounded-3xl bg-white/85 backdrop-blur-2xl border border-rose-300/70 shadow-2xl shadow-rose-200/60 p-8 space-y-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-2xl font-black text-rose-700">❌ 거절된 상품 목록</h2>
                      <button onClick={() => fetchSellData()}
                        className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200">🔄 새로고침</button>
                    </div>
                    <p className="text-xs text-slate-500">거절 사유가 판매자에게 자동 표시되며, 판매자가 직접 삭제하거나 수정하여 재심사 요청할 수 있습니다.</p>
                    {(() => {
                                ...
                                <div className="flex-1">
                                  ...
                                  {r.rejectionReason && (
                                    <div className="mt-2 rounded-xl border border-rose-300 bg-white/80 p-3">
                                      <div className="text-xs font-bold text-rose-700">거절 사유</div>
                ...
                {adminSubTab === 'categories' && (
                  <div className="rounded-3xl bg-white/85 backdrop-blur-2xl border border-purple-300/70 shadow-2xl shadow-purple-200/60 p-8 space-y-5">
                    ...
                    <p className="text-xs text-slate-500">판매자는 이 목록 중에서만 카테고리를 선택합니다. 비활성화하면 새 등록에 노출되지 않습니다.</p>

                    {/* 추가 폼 */}
                    <div className="rounded-2xl border border-violet-200 bg-violet-50/50 p-4 space-y-3">
                      <div className="text-sm font-black text-violet-700">➕ 새 카테고리 추가</div>
                      <div className="grid gap-3 md:grid-cols-4">
                        <label className="grid gap-1">
                          ...
                          <select value={newCategoryType} onChange={(e) => setNewCategoryType(e.target.value as typeof newCategoryType)}
                            ...
                            <option value="physical">📦 일반상품</option>
                    ...
                    {adminCategoriesLoading ? (
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">등록된 카테고리가 없습니다.</div>
                    ) : (
                      <div className="space-y-2">
                        {(['physical','service','food','digital'] as const).map((pt) => {
                          const items = adminCategories.filter((c) => c.product_type === pt)
                          if (items.length === 0) return null
                          const typeLabel: Record<string,string> = { physical: '📦 일반상품', service: '📅 서비스예약', food: '🍽 음식매장', digital: '💾 디지털' }
                          return (
                ...
                {adminSubTab === 'mail' && (
                  <div className="rounded-3xl bg-white/85 backdrop-blur-2xl border border-purple-300/70 shadow-2xl shadow-purple-200/60 p-8 space-y-4">
                    <h2 className="text-2xl font-black">📨 이메일 큐 수동 처리</h2>
                    <p className="text-sm text-slate-600">
                      승인/주문 확정 시 자동 적재되는 이메일 전송 대기열을 지금 일괄 발송합니다.<br/>
                      일반적으로 5분마다 자동 처리되지만, 즉시 발송이 필요할 때 사용합니다.
                    </p>
                    <div className="flex flex-wrap gap-3">
                      <button onClick={handleProcessEmailQueue}
                        className="rounded-xl bg-gradient-to-r from-sky-500 to-indigo-500 px-4 py-2 text-sm font-black text-white shadow-lg">📤 대기열 즉시 발송</button>
                ...
                {/* ✨ Patch v15.2: review 부탭일 때만 기존 '관리자 검토 목록' 노출 */}
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="mt-5 grid gap-3 rounded-2xl border border-purple-200/80 bg-white/80 p-4">
                ...
                {/* ✨ Patch v16.0 / v17.0: DAO2 환산 실시간 표시 (폴백 포함) */}
                <div className="rounded-xl bg-gradient-to-r from-violet-50 to-fuchsia-50 border border-violet-200 px-3 py-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-black text-violet-700">💰 결제 금액 (WebKey DAO2)</span>
                    <span className="text-lg font-black bg-gradient-to-r from-violet-600 via-fuchsia-600 to-pink-500 bg-clip-text text-transparent">
                      {quoteDao2Loading && !quoteIsFallback
                        ...
                        ? `${(Number(quoteDao2Amount as bigint) / 1e18).toFixed(4)} DAO2`
                        : '—'}
                    </span>
                  </div>
                  {quoteIsFallback && (
                    <div className="mt-1 text-[10px] text-amber-700">
                      ⚠️ 추정 가격 (네트워크 시세 조회 실패) — 실제 결제 금액은 지갑 승인 시 표시됩니다
                    </div>
                  )}
                </div>
