...
const getLatestId = async (table: string) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === 'prime') {
      ...
      const existingId = await getLatestId('prime_settings')
    ...
    if (mode === 'marketplace') {
      const payload = {
        ...
        dao2_price_usdt6:
          body.dao2PriceUsdt6 === null || body.dao2PriceUsdt6 === undefined
            ? null
            : safeNumber(body.dao2PriceUsdt6),
        storage_mode: safeString(body.storageMode) || 'hybrid',
        notes: safeString(body.notes),
        updated_at: new Date().toISOString(),
      }

      const existingId = await getLatestId('marketplace_settings')
      const query = existingId
        ? supabase
